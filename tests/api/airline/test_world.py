...
def test_named_scenario_emits_one_integrated_sensor() -> None:
    ...
    sensors = [
        ...
        if item.type == "sensor.tripped"
        and item.actor_id == "sensor:integrated_hub_disruption"
    ]
    assert event.type == "airline.hub_disruption.detected"
    assert len(sensors) == 1
    assert (
        sensors[0].payload["workflow_type"]
...
def test_records_are_versioned_journal_backed_and_operationally_linked() -> None:
    ...
    assert all(
        ...
        and record.last_event_id in event_ids
...
def test_registered_scene_is_bounded_and_matches_rendered_collections() -> None:
    ...
    assert all(
        "Synthetic" in location["label"] for location in scene["locations"]
    )
    assert {
        layer["state_key"] for layer in scene["layers"]
    } <= world.render_state().keys()
