...
# Hero domestic high-value transfer: Midlands Fulfilment Hub -> London Central,
# Apex X1 hero SKU. Domestic (GB-to-GB), not cross-border, per the Electronics
# world (see verticals/electronics/world.py, verticals/electronics/sensors.py).
...
class _Context:
    instance_id = "electronics-instance-1"
    ...
    def __init__(
        ...
        *,
        ...
        case = ELECTRONICS_REFERENCE_CASES[workflow_type]
        observation = {
            ...
            "as_of_sim_time": 42.0,
            "skills": list(ELECTRONICS_DOMAINS[workflow_type].skills),
            "mcp_tools": [f"electronics-{workflow_type}"],
            "authority": {
    ...
    def call_activity(self, name, payload):
        ...
        durable = import_module("verticals.electronics.durable")
        if name == "electronics_evidence_activity_trigger":
            return durable.electronics_evidence_activity(payload)
        if name == "electronics_decision_activity_trigger":
            return durable.electronics_decision_activity(payload)
        if name == "electronics_command_activity_trigger":
            return durable.electronics_command_activity(payload)
    ...
    def task_any(self, _tasks):
        return self.timer if self._force_timeout else self.approval


def _drive(context: _Context) -> dict[str, Any]:
    durable = import_module("verticals.electronics.durable")
    generator = durable.electronics_orchestration(context)
    sent: Any = None
    while True:
        try:
...
@pytest.mark.parametrize("workflow_type", tuple(ELECTRONICS_PROCESS_PROFILES))
...
def test_domestic_hero_waits_for_exact_authority_event() -> None:
    context = _Context(
        "inventory-rebalancing",
        approval={
            "decision": "approve",
    ...
    # Truthful amount/category/actor evidence carried into the suspend context.
    suspend_payloads = [
        payload
        for name, payload in context.calls
        if name == "checkpoint_activity_trigger"
        and payload["payload"].get("reason") == "awaiting_approval"
    ]
    assert suspend_payloads
    request = suspend_payloads[0]["payload"]["context"]["request"]
    assert request["amount"] == pytest.approx(35_976.0)
    assert request["category"] == "inventory-rebalancing"
...
def test_typed_command_propagates_story_id(workflow_type: str) -> None:
    ...
    assert result["command"]["payload"]["story_id"] == "electronics-launch-shock-42"
...
def test_hitl_timeout_fails_closed_without_command() -> None:
    context = _Context("inventory-rebalancing", force_timeout=True)

    result = _drive(context)

    assert result["status"] == "timeout"
    assert result["command"] is None
    assert context.timer.cancelled is False
...
def test_command_activity_rejects_missing_real_actor_evidence() -> None:
    ...
    with pytest.raises(ValueError, match="actor_ids"):
        durable.electronics_evidence_activity(payload)


def test_command_activity_requires_evidence_digest_and_retains_story_id() -> None:
    durable = import_module("verticals.electronics.durable")
    context = _Context(
        "returns-disposition",
        approval={
        ...
        story_id="electronics-returns-42",
    )
    payload = {
        **context.get_input(),
    ...
    with pytest.raises(ValueError, match="evidence digest"):
        durable.electronics_command_activity(payload)

    payload["evidence"] = {"evidence_digest": "sha256:deadbeef"}
    result = durable.electronics_command_activity(payload)
    assert result["command"]["payload"]["story_id"] == "electronics-returns-42"
    assert (
        result["command"]["payload"]["approval_reference"] == "HITL-RETURN-001"
    )


def test_command_activity_requires_correct_approval_persona() -> None:
    durable = import_module("verticals.electronics.durable")
    context = _Context(
        "returns-disposition",
        approval={"decision": "approve", "persona": "fulfilment_manager"},
    )
    payload = {
        **context.get_input(),
        "instance_id": context.instance_id,
        "skill_outputs": {},
        "evidence": {"evidence_digest": "sha256:deadbeef"},
        "approval": {"decision": "approve", "persona": "fulfilment_manager"},
    }

    with pytest.raises(ValueError, match="returns_operations_manager"):
        durable.electronics_command_activity(payload)
...
def _fake_result(payload: dict[str, Any], tool_calls: list[dict[str, Any]]) -> dict[str, Any]:
    return {
...
def test_hybrid_mode_uses_ghcp_session_for_selected_workflow(
    ...
    payload = _live_payload(
        context,
        instance_id="electronics-live-instance",
        phase="Assess Demand and Constraints",
    )
    ...
    async def fake_run_agent_session(prompt: str, **kwargs):
        ...
        return _fake_result(
            payload,
            [
                {"name": "report_intent", "success": True},
                {"name": "electronics_read_inventory", "success": True},
            ],
        )

    monkeypatch.setenv("ZAVA_ELECTRONICS_AGENT_MODE", "hybrid")
    monkeypatch.setenv(
        "ZAVA_ELECTRONICS_LIVE_WORKFLOWS",
        "inventory-rebalancing,markdown-governance",
    )
    monkeypatch.setattr(durable, "run_agent_session", fake_run_agent_session)

    result = durable.electronics_decision_activity(payload)

    assert result["reasoning"].startswith("Live agent")
    assert captured["workflow_id"] == payload["workflow_id"]
    assert captured["instance_id"] == "electronics-live-instance"
...
def test_hybrid_mode_keeps_unselected_workflow_deterministic(
    ...
    payload = _live_payload(
        context,
        instance_id="electronics-deterministic-instance",
...
def test_hybrid_mode_fails_closed_when_tool_evidence_missing(monkeypatch) -> None:
    durable = import_module("verticals.electronics.durable")
    context = _Context("inventory-rebalancing")
    payload = _live_payload(
        context,
        instance_id="electronics-missing-evidence",
        phase="Assess Demand and Constraints",
    )

    async def fake_run_agent_session(prompt: str, **kwargs):
        result = _fake_result(payload, [])
        del result["_raw_tool_calls"]
        return result

    monkeypatch.setenv("ZAVA_ELECTRONICS_AGENT_MODE", "hybrid")
    monkeypatch.setattr(durable, "run_agent_session", fake_run_agent_session)

    with pytest.raises(ValueError, match="_raw_tool_calls|tool evidence|tool call"):
        durable.electronics_decision_activity(payload)


def test_hybrid_mode_fails_closed_when_zero_business_tool_calls(monkeypatch) -> None:
    durable = import_module("verticals.electronics.durable")
    context = _Context("inventory-rebalancing")
    payload = _live_payload(
        context,
        instance_id="electronics-zero-calls",
        phase="Assess Demand and Constraints",
    )
    ...
    with pytest.raises(ValueError, match="electronics_read_inventory"):
        durable.electronics_decision_activity(payload)


def test_hybrid_mode_fails_closed_when_business_tool_call_duplicated(
    monkeypatch,
) -> None:
    durable = import_module("verticals.electronics.durable")
    context = _Context("inventory-rebalancing")
    payload = _live_payload(
        context,
        instance_id="electronics-duplicate-calls",
        phase="Assess Demand and Constraints",
    )
...
def test_hybrid_mode_fails_closed_when_wrong_tool_called(monkeypatch) -> None:
    durable = import_module("verticals.electronics.durable")
    context = _Context("inventory-rebalancing")
    payload = _live_payload(
        context,
        instance_id="electronics-wrong-tool",
        phase="Assess Demand and Constraints",
    )
    ...
    async def fake_run_agent_session(prompt: str, **kwargs):
        return _fake_result(
            payload,
            [{"name": "electronics_prepare_inventory_transfer", "success": True}],
        )
...
def test_hybrid_mode_fails_closed_when_business_tool_call_failed(monkeypatch) -> None:
    durable = import_module("verticals.electronics.durable")
    context = _Context("inventory-rebalancing")
    payload = _live_payload(
        context,
        instance_id="electronics-failed-call",
        phase="Assess Demand and Constraints",
    )
    ...
    async def fake_run_agent_session(prompt: str, **kwargs):
        return _fake_result(
            payload,
            [{"name": "electronics_read_inventory", "success": False}],
        )
...
def test_hybrid_mode_allows_metadata_calls_alongside_single_business_call(
    monkeypatch,
) -> None:
    durable = import_module("verticals.electronics.durable")
    context = _Context("inventory-rebalancing")
    payload = _live_payload(
        context,
        instance_id="electronics-metadata-plus-business",
        phase="Assess Demand and Constraints",
    )
