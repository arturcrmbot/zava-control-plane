...
def test_decided_on_materialises_rels(graph: EntityGraph) -> None:
    ...
    # Phase 1.5: DECIDED_ON sharded into per-target-kind tables (Kuzu 0.6.1
    # doesn't allow multi-pair rel tables). Person targets land in
    # DECIDED_PERSON.
    neighbours = graph.linked(decision_id, "decided_person")
    assert len(neighbours) == 1
    assert neighbours[0]["node"]["id"] == "PERSON-EMP-0042"
    assert neighbours[0]["rel"] == "DECIDED_PERSON"
...
def test_decided_on_writes_multiple_rels(graph: EntityGraph) -> None:
    ...
    rels = graph.linked(decision_id, "decided_person")
    assert len(rels) == 3
...
def test_decided_on_routes_mixed_kinds_to_per_kind_tables(graph: EntityGraph) -> None:
    """When ``decided_on`` carries ids of mixed kinds, each id is routed
    to its respective ``DECIDED_<KIND>`` table — locks the new schema's
    multi-target contract added in Phase 1.5 (entity-data honesty)."""
    graph.upsert(EntityWrite(kind="Person", id="PERSON-MIXED-1", attrs={"name": "P"}))
    graph.upsert(EntityWrite(kind="Money", id="MONEY-MIXED-1",
                             attrs={"kind": "invoice", "amount": 100.0, "currency": "GBP"}))
    ...
    decision_id = _record(
        ...
        phase="approve",
        persona_role="cfo",
        decided_on=("PERSON-MIXED-1", "MONEY-MIXED-1"),
    )

    person_rels = graph.linked(decision_id, "decided_person")
    money_rels = graph.linked(decision_id, "decided_money")

    assert len(person_rels) == 1
    assert person_rels[0]["node"]["id"] == "PERSON-MIXED-1"
    assert person_rels[0]["rel"] == "DECIDED_PERSON"

    assert len(money_rels) == 1
    assert money_rels[0]["node"]["id"] == "MONEY-MIXED-1"
    assert money_rels[0]["rel"] == "DECIDED_MONEY"

    # And the unfiltered linked() returns both rows, one per kind.
    all_rels = graph.linked(decision_id)
    rel_names = sorted(r["rel"] for r in all_rels)
    assert rel_names == ["DECIDED_MONEY", "DECIDED_PERSON"]
