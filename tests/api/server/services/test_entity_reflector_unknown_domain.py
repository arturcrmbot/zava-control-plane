...
Uses ``not-a-real-domain-type`` (a synthetic id that will never appear
in the registry) since real POC1/POC2 domains (``expense-claim``,
``hiring``) were wired into the substrate by pitch-a4.
"""
...
_UNKNOWN_TYPE = "not-a-real-domain-type"
...
def test_reflector_silently_skips_unknown_workflow_type(tmp_path: Path) -> None:
    # Sanity: the synthetic type really is unregistered.
    assert _UNKNOWN_TYPE not in PROJECTIONS
    ...
    store.upsert_workflow(_make_workflow("WF-UNKNOWN", _UNKNOWN_TYPE))
    ...
    try:
        # Must not raise.
        event_bus.emit(
            FleetEvent(type="workflow.completed", workflow_id="WF-UNKNOWN")
        )
        ...
        assert graph.get("PERSON-WF-UNKNOWN") is None
        assert graph.get("WF-UNKNOWN") is None
    finally:
        reflector.aclose()
