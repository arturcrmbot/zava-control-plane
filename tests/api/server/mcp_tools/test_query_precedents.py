...
def test_happy_path_returns_rows():
    ...
    assert len(rows) == 2
    assert graph.last_cypher == _generic_query(5)
...
def test_generic_query_inlines_limit_and_filters_decided_shards():
    cypher = _generic_query(7)
    # Limit must be inlined as a literal (Kuzu 0.6.1 rejects $limit).
    assert "LIMIT 7" in cypher
    assert "$limit" not in cypher
    # Must filter via label(r) IN [...] across every DECIDED_<KIND> shard
    # (post-Phase-1.5 a hard-coded :DECIDED_ON would match nothing).
    assert "label(r) IN [" in cypher
    for shard in ("DECIDED_PERSON", "DECIDED_MONEY", "DECIDED_ASSET",
                  "DECIDED_ORG", "DECIDED_PERIOD", "DECIDED_PLACE",
                  "DECIDED_ON"):
        assert f"'{shard}'" in cypher
...
def test_deny_list_rejects_write_verbs(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="write/DDL keyword"):
        qp._load_template("rogue", "phase", 10)
...
def test_existing_template_loads(tmp_path: Path, monkeypatch):
    ...
    tmpl.write_text(
        "MATCH (d:Decision)-[:DECIDED_ON]->(e {id: $entity_id}) RETURN d LIMIT 10",
        encoding="utf-8",
    )
    monkeypatch.setattr(qp, "_TEMPLATE_DIR", tmp_path)
    cypher = qp._load_template("expense-claim", "approval", 10)
...
def test_workflow_type_phase_routes_to_template(tmp_path: Path, monkeypatch):
    ...
    tmpl.write_text("MATCH (d:Decision) RETURN d LIMIT 10", encoding="utf-8")
    monkeypatch.setattr(qp, "_TEMPLATE_DIR", tmp_path)
    graph = _FakeGraph([])
    tool = qp.make_query_precedents_tool(graph)
    tool("role", "ent", limit=3, workflow_type="wt", phase="phase")
    assert graph.last_cypher == "MATCH (d:Decision) RETURN d LIMIT 10"
