"""api/shared/personas.py — persona declarations.
...
from api.shared.vertical_loader import select_vertical as _select_vertical
...
# --------------------------------------------------------------------------
# autonomous-domain-insights v1.1 (F1): per-persona display hue palette.
# Grouped by function family — the UI uses these hues to give each
# persona a distinct visual identity (DecisionTicker name colouring,
# workflow particle gate-flash). Personas not listed render in neutral
# grey on the frontend. Keep palette tasteful and cosmic-lens friendly.
# --------------------------------------------------------------------------
...
PERSONAS = {
    role: (
        _dataclasses.replace(p, display_color=_DISPLAY_COLORS[role])
        if role in _DISPLAY_COLORS
        else p
    )
    for role, p in PERSONAS.items()
}

ALL_PERSONAS = MappingProxyType(dict(PERSONAS))
_TELCO_PERSONA_ROLES = {
    "cs_director",
    "cs_account_director",
    "cs_manager",
    "cs_specialist",
...
_TELCO_ONLY_PERSONAS = _TELCO_PERSONA_ROLES - {"delivery_lead"}

if _select_vertical(os.environ)[0] == "telco":
    PERSONAS = {
        role: ALL_PERSONAS[role]
        for role in _TELCO_PERSONA_ROLES
    }
else:
    PERSONAS = {
        role: persona
        for role, persona in ALL_PERSONAS.items()
        if role not in _TELCO_ONLY_PERSONAS
    }
