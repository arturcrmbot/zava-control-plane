"""api/shared/agents.py — machine-agent declarations.
...
ALL_AGENTS = MappingProxyType(dict(AGENTS))
_TELCO_AGENT_IDS = {
...
if _select_vertical(os.environ)[0] == "telco":
    AGENTS = {
        ...
        for agent_id in _TELCO_AGENT_IDS | {"reflector.entity_reflector"}
    }
else:
    AGENTS = {
        agent_id: agent
        for agent_id, agent in ALL_AGENTS.items()
        if agent_id not in _TELCO_AGENT_IDS
    }
