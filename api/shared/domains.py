"""api/shared/domains.py — Domain registry declarations.
...
ALL_DOMAINS = MappingProxyType(dict(DOMAINS))
_TELCO_WORKFLOW_TYPES = {
    "network-incident",
...
if _select_vertical(os.environ)[0] == "telco":
    DOMAINS = {
        ...
        for workflow_type in _TELCO_WORKFLOW_TYPES
    }
else:
    DOMAINS = {
        workflow_type: domain
        for workflow_type, domain in ALL_DOMAINS.items()
        if workflow_type not in _TELCO_WORKFLOW_TYPES
    }
    DOMAINS["creative-campaign"] = replace(
        DOMAINS["creative-campaign"],
