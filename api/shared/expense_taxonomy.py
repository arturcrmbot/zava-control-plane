...
verdict / category / flavour / tier drift between the layers."""
...
ReceiptFlavour = Literal[
    "correct", "wrong-amount", "wrong-date", "wrong-vendor",
    "missing-line-item", "missing-receipt",
]

RECEIPT_FLAVOURS: tuple[ReceiptFlavour, ...] = (
    "correct", "wrong-amount", "wrong-date", "wrong-vendor",
    "missing-line-item", "missing-receipt",
)

EscalationTier = Literal["warning", "escalation", "major-violation"]

ESCALATION_TIERS: tuple[EscalationTier, ...] = ("warning", "escalation", "major-violation")
