"""api/shared/authority.py — delegated-authority declarations.
...
ALL_AUTHORITY = MappingProxyType(dict(AUTHORITY))
_TELCO_AUTHORITY_ROLES = {
    "cs_specialist",
    "cs_manager",
    "cs_account_director",
    "cs_director",
...
_TELCO_ONLY_AUTHORITY = _TELCO_AUTHORITY_ROLES - {"delivery_lead"}

if _select_vertical(os.environ)[0] == "telco":
    AUTHORITY = {
        role: ALL_AUTHORITY[role]
        for role in _TELCO_AUTHORITY_ROLES
    }
else:
    AUTHORITY = {
        role: row
        for role, row in ALL_AUTHORITY.items()
        if role not in _TELCO_ONLY_AUTHORITY
    }
