...
allowed-tools:
workflow_label: Marketing — production
external_event: producer_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "producer_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within producer delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside producer delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **producer** for the **Marketing — production** workflow.
...
When `producer` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
