...
allowed-tools:
workflow_label: Technology — support
external_event: support_engineer_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "support_engineer_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within support_engineer delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside support_engineer delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **support_engineer** for the **Technology — support** workflow.
...
When `support_engineer` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
