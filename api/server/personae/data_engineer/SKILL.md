...
allowed-tools:
workflow_label: Data — engineering
external_event: data_engineer_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "data_engineer_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within data_engineer delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside data_engineer delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **data_engineer** for the **Data — engineering** workflow.
...
When `data_engineer` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
