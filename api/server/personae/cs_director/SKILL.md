...
allowed-tools:
workflow_label: Customer Success — director
external_event: cs_director_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "cs_director_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within cs_director delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside cs_director delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **cs_director** for the **Customer Success — director** workflow.
...
When `cs_director` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
