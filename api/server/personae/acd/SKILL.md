...
decision_policy: |
    ...
    action = (context or {}).get("action") or "acd_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within acd delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside acd delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **acd** for the **Marketing — creative** workflow.
...
When `acd` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
