...
external_event: mid_creative_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "mid_creative_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within mid_creative delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside mid_creative delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **mid_creative** for the **Marketing — creative** workflow.
