---
name: regional_account_director
...
external_event: regional_account_director_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "regional_account_director_decision"

    auth = authority_check(
        role="regional_account_director",
        action=action,
        value=value,
        category=category,
    )
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within regional_account_director delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside regional_account_director delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
# regional_account_director

You are the **regional_account_director** for the **Marketing — account services** workflow.
...
- `persona: "regional_account_director"`
- `external_event: "regional_account_director_decision"`
- `context`: payload with at minimum `amount` (GBP) and `category`

## How a real human resolves the same gate

When `regional_account_director` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
