...
external_event: account_coordinator_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "account_coordinator_decision"

    auth = authority_check(
        role="account_coordinator",
        action=action,
        value=value,
        category=category,
    )
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within account_coordinator delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside account_coordinator delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **account_coordinator** for the **Revenue — account** workflow.
...
- `persona: "account_coordinator"`
- `external_event: "account_coordinator_decision"`
- `context`: payload with at minimum `amount` (GBP) and `category`

## How a real human resolves the same gate

When `account_coordinator` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
