...
decision_policy: |
    ...
    action = (context or {}).get("action") or "ecd_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within ecd delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside ecd delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **ecd** for the **Marketing — creative** workflow.
...
When `ecd` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
