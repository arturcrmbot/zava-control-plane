...
allowed-tools:
workflow_label: Data — analytics
external_event: analytics_engineer_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "analytics_engineer_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within analytics_engineer delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside analytics_engineer delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **analytics_engineer** for the **Data — analytics** workflow.
...
When `analytics_engineer` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
