...
allowed-tools:
workflow_label: Data — analyst
external_event: analyst_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "analyst_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within analyst delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside analyst delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **analyst** for the **Data — analyst** workflow.
...
When `analyst` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
