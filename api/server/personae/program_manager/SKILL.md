...
allowed-tools:
workflow_label: Operations — programme
external_event: program_manager_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "program_manager_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within program_manager delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside program_manager delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **program_manager** for the **Operations — programme** workflow.
...
When `program_manager` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
