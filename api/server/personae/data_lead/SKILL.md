...
allowed-tools:
workflow_label: Data — lead
external_event: data_lead_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "data_lead_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within data_lead delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside data_lead delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **data_lead** for the **Data — lead** workflow.
...
When `data_lead` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
