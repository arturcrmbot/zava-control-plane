...
allowed-tools:
workflow_label: Marketing — planning
external_event: planner_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "planner_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within planner delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside planner delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **planner** for the **Marketing — planning** workflow.
...
When `planner` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
