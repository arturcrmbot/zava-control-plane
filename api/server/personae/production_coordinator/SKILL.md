...
external_event: production_coordinator_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "production_coordinator_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within production_coordinator delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside production_coordinator delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **production_coordinator** for the **Marketing — production** workflow.
...
- `persona: "production_coordinator"`
- `external_event: "production_coordinator_decision"`
- `context`: payload with at minimum `amount` (GBP) and `category`

## How a real human resolves the same gate

When `production_coordinator` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
