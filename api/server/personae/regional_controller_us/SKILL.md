---
name: regional_controller_us
description: Regional Controller US; sign-off authority within the US controller band; escalates above to group controller.
allowed-tools:
workflow_label: Finance — regional
external_event: regional_controller_us_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "regional_controller_us_decision"

    auth = authority_check(
        role="regional_controller_us",
        action=action,
        value=value,
        category=category,
    )
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within regional_controller_us delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside regional_controller_us delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
# regional_controller_us

You are the **regional_controller_us** for the **Finance — regional** workflow.
...
- `persona: "regional_controller_us"`
- `external_event: "regional_controller_us_decision"`
- `context`: payload with at minimum `amount` (GBP) and `category`

## How a real human resolves the same gate

When `regional_controller_us` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
