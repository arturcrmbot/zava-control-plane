...
external_event: contracts_counsel_senior_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "contracts_counsel_senior_decision"

    auth = authority_check(
        role="contracts_counsel_senior",
        action=action,
        value=value,
        category=category,
    )
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within contracts_counsel_senior delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside contracts_counsel_senior delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **contracts_counsel_senior** for the **Legal — contracts** workflow.
...
- `persona: "contracts_counsel_senior"`
- `external_event: "contracts_counsel_senior_decision"`
- `context`: payload with at minimum `amount` (GBP) and `category`

## How a real human resolves the same gate

When `contracts_counsel_senior` is NOT in `PERSONA_AUTO_CLOSE`, the gate stays open
indefinitely.
