...
external_event: junior_creative_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "junior_creative_decision"

    auth = authority_check(
        role="junior_creative",
        action=action,
        value=value,
        category=category,
    )
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within junior_creative delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside junior_creative delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **junior_creative** for the **Marketing — creative** workflow.
