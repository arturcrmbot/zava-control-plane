...
external_event: media_planner_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "media_planner_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within media_planner delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside media_planner delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **media_planner** for the **Marketing — media** workflow.
