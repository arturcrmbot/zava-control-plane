...
async def workflow_timeline(id: str):
    ...
    for p in app_state.store.get_phases(id):
        # A "completed" phase is a milestone — its ts must sort AFTER the spans
        # that ran during it, so anchor on completed_at when available.
        status = getattr(p, "status", "?")
        completed = getattr(p, "completed_at", None)
        started = getattr(p, "started_at", None)
        ts = completed if (status == "completed" and completed) else (started or w.created_at)
        rows.append({
            "ts": float(ts),
            "kind": "phase",
    ...
    for entry in (w.action_ledger or []):
        ...
        # Anchor workflow.started to the workflow's created_at so it always
        # leads the transcript, even when the orchestrator stamps the ledger
        # entry sub-second after the first span begins.
        if action == "workflow.started":
            ts = float(w.created_at)
        rows.append({
            "ts": ts,
    ...
    # When two rows share a timestamp (synthetic workflows compress into a
    # sub-second window), use this tie-breaker so the reading order makes
    # narrative sense: the workflow always starts first; agents/tools run
    # mid-phase; the phase milestone closes; decisions land last.
    _kind_rank = {
        "workflow.started": 0,
        "phase_start": 1,
        "agent": 2,
    ...
    def _rank(r: dict) -> int:
        if r.get("label") == "workflow.started":
            return _kind_rank["workflow.started"]
        if r.get("kind") == "phase":
            return _kind_rank["phase_completed" if r.get("status") == "completed" else "phase_start"]
        return _kind_rank.get(r.get("kind", ""), 5)
