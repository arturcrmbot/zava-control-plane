...
@router.get("/health")
async def health(actor: Actor = Depends(require_actor)):
    if not foundry_client.is_configured():
        return _unconfigured_envelope()
    h = _store.health()
    return {
...
@router.get("/{eval_id}")
async def get_eval(eval_id: str, actor: Actor = Depends(require_actor)):
    # NOTE: per-role projector is intentionally a no-op for evals — the
    # eval store is internal-only telemetry on synthetic prompts and the
    # downstream eval UI needs the full prompt/response_text/context to
    # render the judge rationale. Sensitivity is gated at ingress via
    # ``require_actor``; consider redacting here if real-PII prompts ever
    # land in the store.
    if not foundry_client.is_configured():
        return _unconfigured_envelope()
