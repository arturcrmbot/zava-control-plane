...
async def raise_orchestration_event(instance_id: str, event_name: str, event_data: Any) -> bool:
    """Raise an external event on a running orchestration (resumes wait_for_external_event).

    Returns True on success, False if the orchestration instance does not exist
    (404 from the Durable runtime — typically a zombie left over from an
    Azurite wipe across boots). The caller can use the False return to mark
    the workflow as orphaned and let the recovery sweep clean it up rather
    than leaving it parked at HITL forever.
    """
