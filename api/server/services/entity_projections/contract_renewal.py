...
def project(workflow: Workflow) -> list[EntityWrite | RelWrite | DecisionWrite]:
    ...
    current_value = (c.get("current_annual_value")
                     if "current_annual_value" in c else p.get("current_annual_value")) or 0
    proposed_value = (c.get("proposed_annual_value")
                      if "proposed_annual_value" in c else p.get("proposed_annual_value")) or 0
