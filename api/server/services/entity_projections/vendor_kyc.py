...
def project(workflow: Workflow) -> list[EntityWrite | RelWrite | DecisionWrite]:
    ...
    country = str(v.get("country_of_incorporation") or p.get("country_of_incorporation") or "")
    agency = str(v.get("proposing_agency") or p.get("proposing_agency") or "unknown")
    scenario = str(p.get("scenario") or v.get("scenario") or "")
