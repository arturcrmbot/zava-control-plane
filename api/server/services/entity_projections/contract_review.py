...
def project(workflow: Workflow) -> list[EntityWrite | RelWrite | DecisionWrite]:
    ...
    c = p.get("contract_review") or {}
    contract_id = str(c.get("contract_id") or p.get("contract_id") or workflow.id)
    vendor_name = str(c.get("vendor_name") or p.get("vendor_name") or "unknown")
    contract_type = str(c.get("contract_type") or p.get("contract_type") or "")
    amount = (c.get("amount_gbp") if "amount_gbp" in c else p.get("amount_gbp")) or 0
    deviates = bool(c.get("deviates_from_template",
                          p.get("deviates_from_template", False)))
