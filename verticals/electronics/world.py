...
from verticals.electronics.actors import (
    Brand,
    Customer,
    Delivery,
    InventoryPosition,
...
from verticals.electronics.dynamics import (
    HERO_SALE_TICKS,
...
from verticals.electronics.process_profiles import (
    ELECTRONICS_PROCESS_PROFILES,
    ElectronicsProcessProfile,
)
from verticals.electronics.reference_cases import ELECTRONICS_REFERENCE_CASES
from verticals.electronics.sensors import (
...
from verticals.electronics.trading_shock import TradingShockState


STORE_DATA = (
    ("STORE-UK-LON-01", "London Central", "GB", "London"),
    ("STORE-UK-MAN-01", "Manchester Trafford", "GB", "North West"),
    ("STORE-UK-BHM-01", "Birmingham Bullring", "GB", "West Midlands"),
    ("STORE-UK-LDS-01", "Leeds Trinity", "GB", "Yorkshire and the Humber"),
    ("STORE-UK-BRS-01", "Bristol Cabot", "GB", "South West"),
    ("STORE-UK-GLA-01", "Glasgow Braehead", "GB", "Scotland"),
    ("STORE-UK-CDF-01", "Cardiff Central", "GB", "Wales"),
    ("STORE-UK-BFS-01", "Belfast Boucher", "GB", "Northern Ireland"),
)
DC_DATA = (
    ("DC-UK-MID-01", "Midlands Fulfilment Hub", "GB", "West Midlands"),
    ("DC-UK-SE-01", "South East Delivery Hub", "GB", "South East"),
)
# (brand full name, brand short name for product naming, product category, SKU code prefix)
BRAND_DATA = (
    ("Apex Devices", "Apex", "Gaming Laptop", "APEX"),
    ("Voltix", "Voltix", "Smartphone", "VOLT"),
    ("Nimbus Technologies", "Nimbus", "Tablet", "NIMBUS"),
    ("Northline Electronics", "Northline", "4K Television", "NORTH"),
    ("CrestPoint Audio", "CrestPoint", "Soundbar", "CREST"),
    ("Orbital Gear", "Orbital", "Wireless Headphones", "ORBIT"),
    ("Fenwick Home", "Fenwick", "Smartwatch", "FENWICK"),
    ("BrightArc", "BrightArc", "Smart Speaker", "BRIGHT"),
    ("Solace Living", "Solace", "Home Security Camera", "SOLACE"),
    ("Ferrotech", "Ferrotech", "27-inch Monitor", "FERRO"),
    ("Halcyon Digital", "Halcyon", "Mesh Router", "HALCYON"),
    ("PulseWave Audio", "PulseWave", "Portable Speaker", "PULSE"),
)
FINISH_PAIRS = (
    ("GRAPHITE", "SILVER"),
    ("MIDNIGHT", "STARLIGHT"),
...
SIZES = ("13", "14", "15", "16")
ORDINARY_SKU = "SKU-VOLT-X1-MIDNIGHT-14"
DOMESTIC_DESTINATION_LOCATION = "STORE-UK-MAN-01"
...
class ElectronicsScenario:
    reference_process_types = tuple(ELECTRONICS_PROCESS_PROFILES)
    ...
    def _seed_catalogue(self) -> None:
        relationships = ("owned",) * 4 + ("consignment",) * 4 + ("marketplace",) * 4
        self.brands = {
            f"BRAND-{number:02d}": Brand(
                id=f"BRAND-{number:02d}",
                name=BRAND_DATA[number - 1][0],
                relationship=relationships[number - 1],
            )
            for number in range(1, 13)
        }
        for number in range(1, 25):
            brand_index = (number - 1) % 12
            generation = ((number - 1) // 12) + 1
            brand_id = f"BRAND-{brand_index + 1:02d}"
            _, short_name, category, code_prefix = BRAND_DATA[brand_index]
            style_id = f"{code_prefix}-X{generation}"
            self.styles[style_id] = Style(
                id=style_id,
                brand_id=brand_id,
                name=f"{short_name} X{generation} {category}",
                lifecycle="current-generation",
            )
            finishes = FINISH_PAIRS[(number - 1) % len(FINISH_PAIRS)]
            for finish in finishes:
                for size in SIZES:
                    sku_id = f"SKU-{style_id}-{finish}-{size}"
                    self.skus[sku_id] = SKU(
                        id=sku_id,
                        style_id=style_id,
                        colour=finish,
                        size=size,
                        retail_price_gbp=float(599 + ((number * 53) % 900)),
                    )

    def _seed_people(self) -> None:
        store_list = list(self.stores.values())
        self.customers = {
            f"CUST-{number:04d}": Customer(
                id=f"CUST-{number:04d}",
                home_region=store_list[(number - 1) % len(store_list)].region,
                location_id="OFFSITE",
                status="offsite",
            )
        ...
        for store_index, store_id in enumerate(self.stores):
            for staff_index in range(1, 4):
                ...
                self.staff[staff_id] = Staff(
                    ...
                    role=("sales-advisor" if staff_index < 3 else "store-manager"),
    ...
    def _seed_inventory(self) -> None:
        ...
        source.retail_price_gbp = 1_499.0
        ...
        destination.retail_price_gbp = 1_499.0

        domestic_source = self.inventory[(SOURCE_LOCATION, ORDINARY_SKU)]
        domestic_source.ownership = "owned"
        domestic_source.on_hand = 100
        domestic_source.reserved = 2
        domestic_source.safety_stock = 20
        domestic_destination = self.inventory[
            (DOMESTIC_DESTINATION_LOCATION, ORDINARY_SKU)
        ]
    ...
    def _seed_operations(self) -> None:
        ...
        self.returns["RETURN-0042"] = Return(
            ...
            sku_id="SKU-CREST-X1-ONYX-15",
            location_id="STORE-UK-LON-01",
            status="received",
        )
        self.promotions["PROMO-LAUNCH-01"] = Promotion(
            id="PROMO-LAUNCH-01",
            name="Flagship launch spotlight",
            status="at-risk",
            sku_ids=("SKU-NIMBUS-X1-CHARCOAL-14",),
        )
        self.seller_offers["OFFER-MKT-0003"] = SellerOffer(
            id="OFFER-MKT-0003",
            seller_id="SELLER-03",
            sku_id="SKU-SOLACE-X1-CHARCOAL-13",
            status="active",
        )
        self.process_cases = {
            ...
            for workflow_type, case in ELECTRONICS_REFERENCE_CASES.items()
        }
    ...
    def _evaluate_inventory_sensor(self, cause: SimulationEvent) -> None:
        ...
        story_id = f"electronics-launch-shock-{self.runtime.seed}"
        ...
        detected = self.runtime.emit(
            "retail.launch-shock.detected",
            actor_id="retail:launch-shock",
            target_id=HERO_SKU,
    ...
    def _emit_ready_story_sensors(self, cause: SimulationEvent) -> None:
        ...
        for stage in self.trading_shock.ready_to_trigger():
            profile = ELECTRONICS_PROCESS_PROFILES[stage.workflow_type]
    ...
    def bind_story_workflow(
        ...
        if workflow_type not in ELECTRONICS_PROCESS_PROFILES:
            return
    ...
    def fail_story_workflow(self, workflow_id: str, reason: str) -> None:
        for workflow_type in ELECTRONICS_PROCESS_PROFILES:
    ...
    def _executive_kpis(self) -> dict[str, float]:
        ...
        story_stages = self.trading_shock.view()["stages"]
        completed_stages = sum(
            stage["status"] == "completed" for stage in story_stages
        )
        ...
        return {
            ...
            "destination_available_units": destination.available,
            ...
            "process_completion_pct": round(
                100 * completed_stages / len(story_stages),
                2,
            )
            if story_stages
            else 0.0,
    ...
    def build_observation(
        ...
        *,
        ...
        profile = next(
            (
                candidate
                for candidate in ELECTRONICS_PROCESS_PROFILES.values()
                if candidate.sensor_id == sensor_id
            ),
            None,
        )
        if profile is None:
            raise ValueError(f"unknown Electronics sensor {sensor_id!r}")
        ...
        if profile.workflow_type == "inventory-rebalancing":
            ...
            source_position = self.inventory[(SOURCE_LOCATION, HERO_SKU)]
            auto_value_limit_gbp = 10_000
            transfer_value_gbp = round(
                transfer_quantity * source_position.retail_price_gbp, 2
            )
            cross_border = (
                self._location(SOURCE_LOCATION).country
                != self._location(DESTINATION_LOCATION).country
            )
            observation.update(
                {
                    "measurements": inventory_measurements(self),
                    "transfer_candidate": {
                        ...
                        "ownership": "owned",
                        "expected_source_version": source_position.version,
                    ...
                    "policy": {
                        ...
                        "reason": (
                            f"transfer value GBP {transfer_value_gbp:,.2f} "
                            f"exceeds the GBP {auto_value_limit_gbp:,} "
                            "delegated limit"
                        ),
                        "auto_value_limit_gbp": auto_value_limit_gbp,
    ...
    @staticmethod
    def _tools_for(profile: ElectronicsProcessProfile) -> list[str]:
        tools = {
            "inventory-rebalancing": [
                "electronics_read_inventory",
                "electronics_prepare_inventory_transfer",
            ],
            "demand-spike-response": ["electronics_read_inventory"],
            "promotion-readiness": ["electronics_assess_promotion"],
            "markdown-governance": ["electronics_prepare_markdown_recommendation"],
            "supplier-delay-recovery": ["electronics_prepare_supplier_recovery"],
            ...
            "returns-disposition": ["electronics_prepare_return_disposition"],
        }
        return tools[profile.workflow_type]

    def run_reference_process(self, workflow_type: str) -> dict[str, Any]:
        profile = ELECTRONICS_PROCESS_PROFILES.get(workflow_type)
        if profile is None:
            raise ValueError(f"unknown Electronics process {workflow_type!r}")
    ...
    def _apply_reference_command(
        self,
        profile: ElectronicsProcessProfile,
        command: SimulationCommand,
    ) -> SimulationEvent:
        payload = command.payload
    ...
    def _mutate_reference_state(
        ...
        if profile.workflow_type == "demand-spike-response":
            ...
            self.promotions["PROMO-LAUNCH-01"].status = "ready"
    ...
    def _location(self, location_id: str) -> Location:
        ...
        if location is None:
            raise ValueError(f"unknown Electronics location {location_id!r}")
        return location
    ...
    def _inventory_token_views(self) -> list[dict[str, Any]]:
        visible_skus = {HERO_SKU, ORDINARY_SKU}
