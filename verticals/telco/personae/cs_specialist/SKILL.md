...
allowed-tools:
workflow_label: Customer Success — specialist
external_event: cs_specialist_decision
decision_policy: |
    ...
    action = (context or {}).get("action") or "cs_specialist_decision"
    ...
    if auth.get("allowed"):
        decision = "approve"
        reason = (
            "within cs_specialist delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
        )
    else:
        decision = "escalate"
        reason = (
            "outside cs_specialist delegation per matrix rule " + rule
            + ": " + str(category) + " GBP " + str(value)
...
You are the **cs_specialist** for the **Customer Success — specialist** workflow.
