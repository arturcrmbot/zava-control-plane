"""Banking supporting-process synthetic MCP tools.
...
class ReadCaseEvidenceParams(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)

    observation: CaseObservation


class RankAdmittedCaseOptionsParams(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)
...
@define_tool(
    name="banking_read_case_evidence",
    description="Read supplied versioned synthetic supporting-case evidence.",
)
def banking_read_case_evidence(
    params: ReadCaseEvidenceParams,
) -> ToolResult:
...
@define_tool(
    name="banking_rank_admitted_case_options",
    description=(
        "Return deterministically admitted synthetic supporting-case options "
        "unchanged with bounded ranking context."
    ),
)
def banking_rank_admitted_case_options(
    params: RankAdmittedCaseOptionsParams,
) -> ToolResult:
    return _tool_result(
        {
