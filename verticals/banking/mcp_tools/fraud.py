"""Banking Hero 1 – APP Fraud Reimbursement synthetic MCP tools.
...
class ReadClaimEvidenceParams(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)

    observation: ClaimObservation


class RankAdmittedClaimOptionsParams(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)
...
@define_tool(
    name="banking_read_claim_evidence",
    description="Read supplied versioned synthetic APP fraud claim evidence.",
)
def banking_read_claim_evidence(
    params: ReadClaimEvidenceParams,
) -> ToolResult:
...
@define_tool(
    name="banking_rank_admitted_claim_options",
    description=(
        "Return deterministically admitted synthetic reimbursement options "
        "unchanged with bounded ranking context."
    ),
)
def banking_rank_admitted_claim_options(
    params: RankAdmittedClaimOptionsParams,
) -> ToolResult:
    return _tool_result(
        {
