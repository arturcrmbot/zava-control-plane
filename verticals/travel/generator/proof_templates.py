"""Pack-owned templates for Travel's live E2E proof runners.

The rendered files deliberately live under ``tools/`` because ``make prove``
is the repository-wide entry point.  Their source remains here, inside the
Travel pack generator, and ``generation-manifest.json`` records them as
approved external generated outputs.
"""
from __future__ import annotations


TRAVEL_ZAVA_E2E_PROOF_SH = r"""#!/usr/bin/env bash
set -euo pipefail
...
TRAVEL_ZAVA_E2E_PROOF_PY = r'''#!/usr/bin/env python3
...
TRAVEL_ZAVA_BROWSER_PROOF_MJS = r'''import fs from "node:fs";
