"""Airline Hero 2 – AOG Engineering Recovery synthetic MCP tools.
...
class AogObservation(BaseModel):
...
class ReadAogEvidenceParams(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)

    observation: AogObservation


class RankAdmittedAogOptionsParams(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)
...
def airline_read_aog_evidence(
    params: ReadAogEvidenceParams,
) -> ToolResult:
...
@define_tool(
    ...
    description=(
        "Return deterministically admitted synthetic AOG recovery options "
        "unchanged with bounded ranking context."
    ),
)
def airline_rank_admitted_aog_options(
    params: RankAdmittedAogOptionsParams,
) -> ToolResult:
    return _tool_result(
        {
