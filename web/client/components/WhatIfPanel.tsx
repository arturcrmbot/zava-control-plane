...
export default function WhatIfPanel({ policyId }: { policyId: string }) {
  ...
  return (
    <div className="panel panel-body space-y-3">
      <div className="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">What-if analysis</div>
      <div className="flex gap-2 items-center">
        <input value={value} onChange={e => setValue(e.target.value)} placeholder="proposed value"
          className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded px-2 py-1 text-xs w-40 focus:outline-none focus:ring-2 focus:ring-blue-300" />
      ...
      {result && (
        <div className="text-xs text-slate-700 dark:text-slate-200 space-y-1">
          ...
          {result.impactedWorkflowIds.length > 0 && (
            <div className="text-slate-500 dark:text-slate-400">Impacted: {result.impactedWorkflowIds.join(", ")}</div>
          )}
