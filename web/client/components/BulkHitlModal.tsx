...
export default function BulkHitlModal({ ids, onClose, onConfirm }: {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl p-5 w-[480px] space-y-3">
        <div className="font-semibold text-slate-900 dark:text-slate-100">Bulk resolve {ids.length} exception{ids.length === 1 ? "" : "s"}</div>
        <div className="text-xs text-slate-600 dark:text-slate-300 max-h-40 overflow-auto font-mono bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded p-2">
