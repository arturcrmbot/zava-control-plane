  ...
  return (
    <div className="panel" data-testid="fleet-assignment">
      ...
      <div className="panel-body space-y-1.5">
        {rows.length === 0 && <div className="text-xs text-slate-500 dark:text-slate-400">no executors fired yet</div>}
        {rows.map(([name, info]) => (
          <div key={name} className="flex items-center gap-2 text-sm min-w-0">
            ...
            <span className="text-slate-800 dark:text-slate-100 truncate flex-1 min-w-0" title={name}>{name}</span>
            <span className="shrink-0 text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wide">{info.type.slice(0, 3)}</span>
            <span className="shrink-0 text-[11px] text-slate-500 dark:text-slate-400 tabular-nums">×{info.count}</span>
          </div>
        ))}
      </div>
    </div>
