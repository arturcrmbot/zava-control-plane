    ...
    <div className="panel" data-testid="policy-autonomy">
      ...
      <div className="panel-body space-y-2">
        ...
        {items.map(p => {
          ...
          return (
            <div key={p.id}>
              <div className="text-xs text-slate-700 dark:text-slate-300 leading-snug">{p.description}</div>
              <div className="flex justify-between items-center mt-1">
                <div className="h-1.5 bg-slate-200 dark:bg-slate-700 rounded flex-1 mr-2">
                  <div className="h-1.5 bg-blue-500 dark:bg-blue-400 rounded" style={{ width: `${pct * 100}%` }} />
                </div>
                <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium whitespace-nowrap">{String(p.currentValue)}</span>
              </div>
            </div>
          );
        })}
