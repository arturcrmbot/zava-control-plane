  ...
  return (
    <details data-testid="about-simulation" className="rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
      ...
      <div className="mt-2 max-w-4xl space-y-2 leading-relaxed">
        ...
        <p>Their choices are made by Laya, a small language model running on this laptop: what to spend on, whether to fall for a scam, when to realise and ring the bank, whether to leave after a claim. Laya weighs the options and a random draw picks one, so no two runs are the same. Amounts, dates and limits are ordinary code.</p>
        ...
        <p data-testid="about-numbers" className="text-slate-500 dark:text-slate-400">
          So far: {count(total, "customer choice", "customer choices")}, {total ? pct((life.decided_by_laya ?? 0) / total) : "0%"} by Laya{life.laya_avg_ms ? ` (about ${Math.round(life.laya_avg_ms)} ms each)` : ""}, the rest by simple rules when it was unsure or busy. None of them use cloud AI.
