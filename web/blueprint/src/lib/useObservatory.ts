  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      es.addEventListener("event", (raw) => {
        if (cancelled) return;
        // Drop malformed events rather than letting a single bad payload
        // tear down the listener and silently freeze the dashboard while
        // EventSource keeps the connection open.
        let data: ObservatoryEvent;
        try {
          data = JSON.parse((raw as MessageEvent).data) as ObservatoryEvent;
        } catch (err) {
          console.warn("useObservatory: dropping malformed SSE event", err);
          return;
        }
        try {
          setEvents((prev) => {
          ...
          console.warn("useObservatory: error handling SSE event", err);
        }
      });
