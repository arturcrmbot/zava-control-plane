...
export function useLiveCosmic(): UseLiveCosmicResult {
  ...
  useEffect(() => {
    ...
    async function pollCities() {
      if (cancelled) return;
      const citiesPollMs = mode === "entities" ? 10_000 : CITIES_POLL_MS;
      try {
      ...
      if (!cancelled) timer = window.setTimeout(pollCities, citiesPollMs);
    }
  ...
  // ---- Pulse polling (entities mode only) ----
  useEffect(() => {
      ...
      es.addEventListener("event", (raw) => {
        ...
        try {
          ...
          const flash: CosmicFlash = {
            ...
            // The runtime emits durable.executor.invoked with stage and
            // skill/tool/executor_type fields. These are required by Rockets
            // to dispatch on stage=start and pick the right city.
            stage: data.stage,
            skill: data.skill,
            tool: data.tool,
