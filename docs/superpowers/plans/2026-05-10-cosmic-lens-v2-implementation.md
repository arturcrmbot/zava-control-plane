...
- [ ] **A.QA.3:** Boot stack via `bash scripts/boot-demo.sh`. Open `http://localhost:5175/?view=constellation`. Inject burst. Visually confirm: disc + planets + moons + rockets all visible and moving.
...
- [ ] **Step 5:** Smoke: `curl localhost:3001/api/cities | jq`.
...
- [ ] **Step 3:** Boot stack. Run sustained burst (`while true; do curl -X POST localhost:3001/api/simulator/inject-burst?n=8; sleep 10; done`) for 5 minutes.
...
- **Stack ports:** FastAPI 3001, blueprint preview 5175, Functions host 7071, Azurite 10000–10002.
