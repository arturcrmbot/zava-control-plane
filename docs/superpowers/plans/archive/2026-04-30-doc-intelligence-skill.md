...
Edit `c:\dev\ghcp sdk stuff\local.settings.json`. The `Values` block currently ends with `"FASTAPI_WEBHOOK_URL": "http://localhost:3101/internal/durable-event"`. Append:
...
```json
{
  ...
  "Values": {
    ...
    "FASTAPI_WEBHOOK_URL": "http://localhost:3101/internal/durable-event",
    "AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT": "",
    "AZURE_DOCUMENT_INTELLIGENCE_KEY": ""
  }
}
...
.funcvenv/Scripts/python.exe -m uvicorn api.server.main:app --port 3101 --host 0.0.0.0 &
...
```bash
curl -s -X POST http://localhost:3101/api/simulator/inject -H "content-type: application/json" -d '{}'
```
...
```bash
curl -sL "http://localhost:3101/api/workflows/" | python -c "
import json,sys
...
```bash
curl -sL "http://localhost:3101/api/workflows/EXP-0001" | python -c "
import json,sys
