...
def test_send_book_interview_email_sends_and_records(fake_app_state):
    out = interview_activities.send_book_interview_email_activity({
        "candidate_id": "C-1",
        "token": "TOKEN-123",
        "portal_url": "http://localhost:5174/book?token=TOKEN-123",
        "role_title": "Senior Data Engineer",
    })
...
Open `http://localhost:5174/book?token=NOT-REAL` — should see "Booking unavailable" with the 404 message rendered nicely. Confirm Vite HMR'd the new route (no console errors).
...
# stop any FastAPI (3001) + func host (7071) processes
...
Wait for all five ports (10000, 3001, 7071, 5173, 5174) to respond. Then:

- [ ] **Step 2: Apply a synthetic CV via curl**

```bash
curl -s -X POST http://localhost:3001/api/portal/apply \
...
until curl -s "http://localhost:3001/api/portal/admin/candidate/<CID>" \
...
SCREEN_TOK=$(curl -s http://localhost:3001/api/portal/admin/links \
...
curl -s -X POST "http://localhost:3001/api/portal/voice/<CID>/canned?token=$SCREEN_TOK"
```

- [ ] **Step 4: Open recruiter view + click Invite**

Open `http://localhost:5174/recruiter`, click into the candidate, confirm the **Decision · invite to interview?** panel shows up with an AI rec card. Click **Invite to interview**.
...
Copy the `book_interview` token from the recruiter view's active-tokens or admin/links endpoint. Open `http://localhost:5174/book?token=<TOKEN>` in a new tab. Pick any available slot.
