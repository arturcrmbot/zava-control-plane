...
> **2026-04-28 — code blocks below in Tasks 8, 9, 15, 16, 20, 21 may show the pre-refactor `@traced_tool` / "pre-fetch in Python" pattern in their inline code samples.** Treat the *convention bullets immediately below* and the *post-refactor `policy_search.py` / `agent_rag_classifier.py`* as canonical. If a task body's snippet conflicts, use the new shape.

Carry over from Week 1, plus Week 2 additions:

- **Single agent identity:** every agent executor calls `run_agent_session(prompt, tools=[...])` from `api/functions/graphs/executors/agents/_wrapper.py`. The wrapper opens an ephemeral session with `skill_directories=[skills_dir]` and the caller's tool list; skills are auto-discovered by the SDK. Don't open `CopilotClient` inline at the agent level.
- **Tools are SDK-native.** Each tool is declared with `@define_tool(name="dotted.name", description="...")` from `copilot.tools`, with a Pydantic params model for argument validation. The decorated function returns a `ToolResult(text_result_for_llm=...)`. Read `policy_search.py` and `claim_get_structured.py` for the canonical pattern after the 2026-04-28 retrofit. The model calls them autonomously.
- **Skills declare `allowed-tools` in frontmatter.** The SDK enforces it; the agent executor passes the matching tool instances via `tools=[...]`.
...
- **JSON output from agents:** `run_agent_session` already extracts the first JSON object from the response and returns a parsed dict. Do not add second-pass extraction.
