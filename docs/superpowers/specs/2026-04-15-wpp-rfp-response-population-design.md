...
## 6. Conflicts to resolve before docx populate

The subagent research surfaced these conflicts between sources. Each requires a human decision before the content plan in §5 can be finalised. Decisions feed back into both the docx populate and the questionnaire xlsx.

| # | Conflict | Options | Recommendation |
|---|---|---|---|
| 1 | Primary low-code builder: Copilot Studio (§06, §15 CSVs, `response-technical-sections.md` §9) vs Control Plane UI skill library (§01 CSV 2.1) | (a) Copilot Studio primary, Control Plane UI for ops config only; (b) inverse; (c) hybrid "both primary, different personas" | **(a)** — aligns with Microsoft's product strategy and WPP's low-code MoSCoW. Correct `01-platform-vendor.csv` 2.1 before xlsx submission. |
| 2 | Agent 365 GA May 2026 vs "Can do today" in 8+ CSVs | (a) Reframe as "Entra Agent ID GA today; Agent 365 umbrella GA May 2026, customer-preview today"; (b) retime claims | **(a)** — honest and accurate. The response must cleanly separate Entra Agent ID (independently usable, arguably GA) from the Agent 365 umbrella (May 2026). Update affected CSVs to use this phrasing. |
| 3 | Threadlight "built and demonstrated" cited operationally in §§01, 06, 15, 19, 23, 27 | Verify delivery-readiness with the delivery team before RFP submission. If not ready, downgrade to "accelerator in active development, demonstrable by POC kick-off". | **Verify, then decide.** User must confirm Threadlight is actually demonstrable in POC timeframe. |
| 4 | Foundry Hosted Agents preview vs GA | (a) "Preview today, GA expected Q3 2026"; (b) verify current Microsoft roadmap | **(a)** with roadmap check. Port into §13 Known Constraints table consistently. |
| 5 | `response-technical-sections.md` §18 forward-references cited in §§05, 07, 15, 20, 26 CSVs | §18 does exist (verified) | **No action.** Preserve forward-references; ensure §18 content ships in Appendix B (it is in `response-technical-sections.md`, which Appendix B should port). |
| 6 | Builder taxonomy mismatch: §06 CSV enumerates 6 modes; §15 presents 4 modes + variants | Align to one taxonomy | **Use the 8-row table from `response-technical-sections.md` §9** as canonical; update CSVs to match. |
| 7 | Five-layer Apex model in master §3 is not defined in the WPP vendor brief .docx | (a) Confirm via WPP's Appendix B PDF (Control Plane Diagrams hyperlink in Q&A doc) — fetch and check; (b) reframe master §3 around WPP's actual 2-plane + 2-layer structure | **(a)** — fetch the PDF; if it confirms the 5-layer model, retain; if not, reframe. High-risk claim otherwise. |
| 8 | "L2" referenced in master §3 but no L2 subsection exists | Either add L2 or remove the forward-reference | **User decision.** Likely an editing artefact. |
| 9 | Autonomy dials: runtime-adjustable vs PR-reviewed. User memory notes preference for PR workflow; WPP brief text demands runtime-adjustable. | (a) Embrace runtime-adjustable with strong audit-trail and dual-control framing; (b) push back with PR-reviewed framing; (c) hybrid — cheap/low-risk dials runtime, high-risk dials PR | **(a)** — aligns with WPP's explicit ask. Frame the governance as "runtime speed without runtime chaos" (§5.5 of content plan). The PR-reviewed framing risks scoring lower against the explicit requirement. |
| 10 | MAI-Transcribe-1 GA date, Foundry Guardrails tool-call interception preview-to-GA, Copilot Studio licensing scope | Verify against current MS release notes | **Verify during tool build** — update CSVs and §13 Known Constraints consistently. |
...
- **WPP deadline 2026-04-23** is in 8 days. Scope must be ruthlessly policed. Any spec §6 conflict that requires external verification (Threadlight readiness, Appendix B PDF for five-layer model, Microsoft release-note checks) needs decisions within 48 hours.
- **Content fidelity risk.** The authored content blocks in spec §5 (especially §§5.5 autonomy framing, §3 inline placeholders) are my drafts based on sources. User must review the authored blocks before they ship.
- **Five-layer Apex model.** If we can't confirm via WPP's Appendix B PDF that this model is WPP's own, we must pivot to the 2-plane + 2-layer structure before the docx populate. Late pivot is expensive.
- **Copilot Studio vs Control Plane UI conflict (#1).** The decision ripples across §§5, 9, and multiple CSVs. Must be locked before content writing begins.
- **python-docx known issues.** Complex hyperlink rendering, bookmark preservation, and paragraph-level XML manipulation can be brittle. Test the populator on a sample section before running full document. If python-docx cannot preserve styles reliably, fall back to a simpler strategy: inject plain paragraphs with style name = master default, and let the user manually restyle in Word.
- **Architecture diagrams gap.** Five WPP-required deliverables (Enterprise Arch, C4, Control Plane wireframe, POC1 PRD, POC2 PRD, Delivery Plan, Testimonials) are not produced by this tool. Must be tracked as separate work items.
