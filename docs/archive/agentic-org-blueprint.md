...
> **Companion docs:** [archive/agentic-org-design.md](archive/agentic-org-design.md) is the
> Plane 1 deep-dive that came out of v1 (archived).
>
...
phase, in `plan/archive/feature-agentic-org-phase-N-*.md`.
...
> **What shipped expanded the v0 schema below.** Phase 1–4 of the
> entity-graph-coherence work
> ([`docs/plans/archive/2026-05-12-entity-graph-coherence.md`](plans/archive/2026-05-12-entity-graph-coherence.md))
> grew the substrate to **15 node kinds** (adds `Subsidiary`, `Brand`,
> `Campaign`, `Pitch`, `MediaPlan`, `Account`, `CostCentre`,
> `Workflow`) and **35 rel tables** with a `decided_at TIMESTAMP` on
> every edge. The accounts substrate (`PAYS` / `OWED_BY` /
> `BOOKED_AGAINST` / `COSTED_TO` / `COSTED_TO_BRAND`) and the agency
> edges (`BRAND_OF` / `CAMPAIGN_FOR` / `EXECUTED_BY`) are live. See
> [ARCHITECTURE.md §4](ARCHITECTURE.md#4-entity-graph--reflector--projections)
> for the current canonical list. The v0 schema below remains as the
...
Plan: `plan/archive/feature-agentic-org-phase-1-entity-graph.md`
...
Plan: `plan/archive/feature-agentic-org-phase-2-compose-v4.md`
...
Plan: `plan/archive/feature-agentic-org-phase-3-function-fms.md`
...
Plan: `plan/archive/feature-agentic-org-phase-4-ceo-fm.md`
...
- Plane 1 deep-dive (will be folded into the Phase 1 plan):
  [docs/archive/agentic-org-design.md](archive/agentic-org-design.md)
