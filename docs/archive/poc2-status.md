...
see [`plan/archive/feature-fleet-domain-substrate-1.md`](../plan/archive/feature-fleet-domain-substrate-1.md)
...
- **`web/portal/`** — fully styled candidate portal (Vite, port 5274). Routes:
...
flowchart TB
    ...
    subgraph DEVBOX["Dev box (laptop · localhost)"]
        VITE["Vite dev server :5273<br/>React Control Plane UI · AG-UI host · /reviewer-queue"]
        FASTAPI["FastAPI :3101 · uvicorn<br/>routes / EventBus / SSEHub / StateStore<br/>FleetManagerService (long-lived GHCP session)"]
