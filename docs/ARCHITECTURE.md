...
| [`web/client/`](../web/client/) | `5273` | Internal operator / agent admin | Mounted by repo-root `index.html` + `vite.config.ts`. Routes: `FleetDashboard`, `ExceptionQueue`, `ReviewerQueue`, `PolicyAndAutonomy`, `Analytics`, `Evaluations`, `Economics`, `WorkflowDetail`, `HiringManager`. Hooks: `useSSE`, `useFleetManagerStream`, `useWorkflows`, `useExceptions`, `useOrchestrationStream`. |
