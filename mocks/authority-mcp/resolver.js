...
const WILDCARD = "*";
function fieldMatches(ruleValue, requestValue) {
...
function valueInBand(band, value) {
...
function isMalformedRule(rule, idx) {
    if (!rule.rule_id)
        return `rule[${idx}] missing rule_id`;
    if (!rule.action)
        return `rule[${rule.rule_id}] missing action`;
    if (!rule.approver_role)
        return `rule[${rule.rule_id}] missing approver_role`;
    if (!rule.value_band_gbp)
        return `rule[${rule.rule_id}] missing value_band_gbp`;
...
export function resolve(matrix, request) {
    for (let i = 0; i < matrix.length; i++) {
        ...
        if (rule.action !== request.action)
            continue;
        if (!fieldMatches(rule.category, request.category ?? null))
            continue;
        if (!fieldMatches(rule.business_unit, request.business_unit ?? null))
            continue;
        if (!fieldMatches(rule.geography, request.geography ?? null))
            continue;
        if (!fieldMatches(rule.requester_role, request.requester_role ?? null))
            continue;
        if (!valueInBand(rule.value_band_gbp, request.value ?? null))
            continue;
        return {
            matched: true,
...
export function checkAuthority(matrix, request) {
