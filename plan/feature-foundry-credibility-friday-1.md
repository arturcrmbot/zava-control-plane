...
returned a live conn string keyed `InstrumentationKey=191e21d7-c49a-4523-bbb3-e2f2349d931f`.
...
- **SEC-001**: The App Insights connection string is a secret; commit only to `.env` and `api/functions/local.settings.json` (both gitignored). `.env.example` keeps the empty placeholder. Storage account access uses `DefaultAzureCredential` against the existing tenant (`AZURE_TENANT_ID=16b3c013-d300-468d-ac64-7eda0820b6d3`); no key-auth.
...
- **DEP-002**: Existing Azure tenant `16b3c013-d300-468d-ac64-7eda0820b6d3` with `AzureCliCredential` working against it (already used by `ocr_extract`, `avatar_render`, and the existing Foundry eval pipeline).
...
- [plan/feature-fleet-domain-substrate-1.md](feature-fleet-domain-substrate-1.md) — prior plan of similar shape; precedent for the per-phase task-table format.
